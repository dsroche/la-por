...
int main(int argc, char* argv[]) {
	...
	for (int i = 0; i < m; i++) {
		...
		ssize_t num_read = pread(fileno(fin), raw_row, bytes_per_row, bytes_per_row * i);
		assert (num_read > 0);
		...
		// accumulate accross one row, 56 bytes (8 chunks) at a time
		...
		if (++accum_count >= MAX_ACCUM_P || i == m-1) {
